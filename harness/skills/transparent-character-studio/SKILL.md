...
3. For legacy Bash, use `bun run import-bash -- <file> <name>`. Never execute the supplied script.
4. Run `bun run validate -- <name>` and fix source errors before rendering.
5. Run `bun run render -- <name>`, then confirm the reported `exports/<name>/` paths exist.
