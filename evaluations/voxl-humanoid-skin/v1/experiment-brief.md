...
- Synthetic image references, baseline documents, and deterministic editable/protected/immutable revision masks are materialized only when `bun run eval:assets check` verifies their committed recipes, bytes, paths, formats, dimensions, and SHA-256 values.
- The pinned Image API candidate remains pending because its public training-data disclosure is not a model-specific licensed-source inventory.
- The provenance-oriented Firefly candidate remains pending because the current Image Model 5 API is not immutably versioned, the documented retention needs product/privacy acceptance, enterprise entitlement and price are unverified, and the catalogued configuration does not claim localized-revision support.
- Both candidate dossiers and public configurations are SHA-256 locked. No provider is admitted and no API call is authorized.
